...
case class Population(generation: Int,
  ...
  def rotate(list: List[Double], double: Double) = (list :+ double).takeRight(maxRotate)
  ...
  def nextGeneration(a: ActorSelection)(implicit fitnessFunction: FitnessFunction,
                                        ...
                                        ec: ExecutionContext): Population = {
    var temp = this.crossOver(a, ec)
    val oldBest = this.individuals.head
    var newBest = temp.individuals.head
    val bitsToMutate = 5
    var hc = false
    val size = oldBest.chromosome.genes.size

    if (this.lastResults.sum < 0.005 && this.lastResults.length >= maxRotate) {
      var useHc = true
      var f = oldBest.fitness
      while (useHc) {
        val start = if (Random.nextInt(20) < 1) 0 else Math.max(0, size - 4)

        Range(start, size).map { gene =>
          temp = temp.hillClimb(a, ec, temperature, bitsToMutate, gene)

        }
        temp = temp.copy(lastResults = List())
        newBest = temp.individuals.head
        hc = true
        useHc = f < newBest.fitness
        f = newBest.fitness
        println(
          "HC total result: oldBestF " + oldBest.fitness + " bew best fitness" + newBest.fitness + " generated by " + newBest.generatedBy)
      }
    }
  ...
  def addGene(a: ActorSelection)(implicit ec: ExecutionContext): Population = {
    println("adding gene")
  ...
  def calc(cList: List[(Chromosome, String)], a: ActorSelection)(implicit ec: ExecutionContext): List[IndividualState] = {
  ...
  private def hillClimb(a: ActorSelection, ec: ExecutionContext, temperature: Temperature, bitsToMutate: Int, gene: Int)(
    ...
    /*
    while (hillClimber.fitness > startingPoint.fitness || i < 3) {

      startingPoint = hillClimber
      hillClimber = hc(startingPoint)
      bestSoFar = if (startingPoint.fitness > hillClimber.fitness) startingPoint else hillClimber
      //println("Testing again gene " + gene + ", try " + i + " sp " + startingPoint.fitness)
      i = i + 1
    } */

    //println("BIF " + bestIndividual.fitness + " < BSFF" + bestSoFar.fitness)
...
object Population {
  ...
  def randomGeneration()(implicit fitnessFunction: FitnessFunction, dimension: ImageDimensions, configuration: Configuration): Population = {
    ...
    Range(0, Size).foreach { i =>
      val c: Chromosome = RandomChromosome.generate(Gene.Size, 3)
